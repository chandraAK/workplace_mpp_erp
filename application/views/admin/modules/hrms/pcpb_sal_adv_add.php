...
<section id="main-content">
  <section class="wrapper"> 
    <div class="row">
        <div class="col-lg-12">
            <h3><i class="fa fa-laptop"></i>Paper Cup & Paper Blank Salary Advance Application</h3>
        ...
        <section class="panel">
            ...
            <div class="panel-body">
                    <?php if($status == "Pending For HOD Approval"){ ?>
                        <form class="form-horizontal" id="myForm" method="post" action="<?php echo base_url(); ?>index.php/hrmsc/pcpb_sa_app_hod">
                    <?php }
                     else if($status == "Pending For HR Approval"){ ?>
                        <form class="form-horizontal" id="myForm" method="post" action="<?php echo base_url(); ?>index.php/hrmsc/pcpb_sa_app_hr">
                    <?php } elseif($status == "Pending For Management Approval"){ ?>
                        <form class="form-horizontal" id="myForm" method="post" action="<?php echo base_url(); ?>index.php/hrmsc/pcpb_sa_app_mng">
                    <?php } elseif($status == "Pending For Payment"){ ?>
                        <form class="form-horizontal" id="myForm" method="post" action="<?php echo base_url(); ?>index.php/hrmsc/pcpb_sa_app_pay">
                    <?php }  else if($status == "Approved"){ ?>
                        <!--Do Nothing-->
                        <form class="form-horizontal" id="myForm" method="post" action="">
                    <?php } else { ?>
                        <form class="form-horizontal" id="myForm" method="post" action="<?php echo base_url(); ?>index.php/hrmsc/pcpb_sal_adv_entry" onsubmit="return reqd();">
                    <?php } ?>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php
                $sql = "select * from salary_adv_pcpb where sal_adv_id = '".$sal_adv_id."'";
...
function sal_adv_cal(emp_id){
    ...
    $("#detail").empty().html('<img src="<?php echo base_url(); ?>assets/images/wait.gif" />');
        ...
        xmlhttp.open("GET","<?php echo base_url(); ?>index.php/hrmsc/pcpb_sal_adv_cal" + queryString,true);
        xmlhttp.send();
}
</script>
...
function reqd(){
    ...
    if(sal_adv_req == "" || sal_adv_req == 0){
        alert("Please enter Salary Advance Amount Required.");
        return false;
    }

    if(sys_cal_advamt == "" || sys_cal_advamt == 0){
        alert("System Calculated Advance Amount Cannot be blank.");
        return false;
    }

    if(Math.round(sal_adv_req * 100) > Math.round(sys_cal_advamt * 100)){
        alert("Advance Required Cannot Be Greater Than System Calculated Advance Amount");
        return false;
    }
}
</script>
